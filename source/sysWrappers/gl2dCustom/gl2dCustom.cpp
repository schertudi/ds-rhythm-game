...
#include "gl2dCustom.h"

namespace gl2dCustom {
...
*/
static v16 g_local_depth = 0; //reset on glBegin2D(), so different objects will have same depth by default
static v16 g_global_depth = 0; //set this to define layer object is drawn on. final depth will = 
...
void glBegin2D( void )
{
	...
	g_local_depth = 0; 	// set depth to 0. We need this var since we cannot disable depth testing
...
******************************************************************************/
void glPutPixel( int x, int y, int color, int depth)
{

	glBindTexture( 0, 0 );
	glColor( color);
	glBegin( GL_TRIANGLES );
		gxVertex3i( x, y, depth );
		gxVertex2i( x, y );
		gxVertex2i( x, y );
	glEnd();
	glColor( 0x7FFF );
...
******************************************************************************/
void glLine( int x1, int y1, int x2, int y2, int color, int depth )
{
...
******************************************************************************/
void glBox( int x1, int y1, int x2, int y2, int color, int depth )
{
...
******************************************************************************/
void glBoxFilled( int x1, int y1, int x2, int y2, int color, int depth )
{
	...
	glColor( color );
	glBegin( GL_QUADS );
		gxVertex3i( x1, y1, depth );		// use 3i for first vertex so that we increment HW depth
						...
						)
{
	...
	glBindTexture( 0,0 );
	glBegin( GL_QUADS );
		glColor( color1 ); gxVertex3i( x1, y1, depth );		// use 3i for first vertex so that we increment HW depth
...
******************************************************************************/
void glTriangle( int x1, int y1, int x2, int y2, int x3, int y3, int color, int depth )
{
...
******************************************************************************/
void glTriangleFilled( int x1, int y1, int x2, int y2, int x3, int y3, int color, int depth )
{
...
void glSprite( int x, int y, int flipmode, const glImage *spr )
{
	...
	glBegin( GL_QUADS );
		...
		gxTexcoord2i( u1, v1 ); gxVertex3i( x1, y1, g_local_depth );	
...
void glSpriteStretchHorizontal(int x, int y, int length_x, const glImage *spr )
{
	...
	glBegin( GL_QUADS );
		...
		gxTexcoord2i( u1, v1 );
		gxVertex3i( x1, y1, g_local_depth );
				   ...
				   )
{
	...
	glBegin( GL_QUADS );
		...
		gxTexcoord2i( u1+uoff, v1+voff ); gxVertex3i( x1, y1, g_local_depth );	
